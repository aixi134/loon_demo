...
京东比价 = type=http-response,pattern=^https:\/\/free\.hulizhushou\.com\/client\/pro-special_training-950386-348\/\d,requires-body=true,binary-body-mode=false,script-path=https://raw.githubusercontent.com/aixi134/loon_demo/main/script/plus.js,timeout=30
[MITM]
...
/**
 * 从视频API响应数据中提取视频信息
 * @param {Object} apiResponse - 视频API的完整响应数据
 * @returns {Object} 包含提取的视频信息对象
 */
function extractVideoInfo(apiResponse) {
    ...
    const videoDetail = apiResponse.data.video_details[0];
    
    try {
        // 格式化总时长(秒)
        const totalSeconds = parseInt(videoDetail.video_duration_second) || 0;
        
        // 解析已观看时间(格式: mm:ss)
        const [minutes, seconds] = (videoDetail.use_time || '00:00').split(':').map(Number);
        const watchedSeconds = (minutes * 60) + seconds;
        ...
        return {
            ...
            totalDuration: videoDetail.video_duration,  // 格式: mm:ss
            totalSeconds: totalSeconds,                // 总秒数
            watchedTime: videoDetail.use_time,         // 已观看时间(mm:ss)
            watchedSeconds: watchedSeconds,            // 已观看秒数
            progressPercent: progressPercent + '%',    // 观看进度百分比
            videoUrl: videoDetail.url,
            thumbnailUrl: videoDetail.banner,
            lastPlayPosition: videoDetail.last_time_point, // 最后播放位置(秒)
            projectInfo: {
        ...
        return {
            error: 'Error parsing video data: ' + error.message,
            data: null
        };
    }
}
...
// 使用示例
//const apiResponse = { /* 你的JSON数据 */ };
//const videoInfo = extractVideoInfo(apiResponse);
